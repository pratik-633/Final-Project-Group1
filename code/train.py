...
def gradient_penalty(critic, real_samples, fake_samples, device=DEVICE):
    ...
    Args:
        critic (_type_): The critic (discriminator) model.
        real_samples (_type_): A batch of real samples from the dataset.
        fake_samples (_type_): A batch of fake samples generated by the generator.
        device (_type_, optional): The device to perform computations on. Defaults to DEVICE.

    Returns:
        _type_: The gradient penalty value.
    """
    ...
    interpolates = epsilon * real_samples + (1 - epsilon) * fake_samples  # interpolation -> this is where you take a point from the line that is drawn btw real and fake - per WGAP-GP paper
    ...
    gradients = gradients.view(batch_size, -1) # flatten gradients to a singl evector per sample
